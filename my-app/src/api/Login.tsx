...
export async function ValidarCredencialesAsync(credentials: LoginCredentials): Promise<LoginResponse> {
  try {
    ...
    const response = await fetch(`https://chat-resorts-builders-calculators.trycloudflare.com/validarcredenciales/`, {
      method: 'POST',
      headers: {
...
export async function ValidarCredencialesDeEmpresaAsync(credentials: LoginCredentials): Promise<LoginResponse> {
  try {
    ...
    const response = await fetch(`https://chat-resorts-builders-calculators.trycloudflare.com/validarcredencialesdeempresa/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
