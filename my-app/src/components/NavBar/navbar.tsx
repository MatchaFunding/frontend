...
function DropdownMenu({ navigate }: { navigate: (path: string) => void }) {
  ...
  return (
    <div className="inline-block text-left" ref={menuRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <button className="bg-white/20 hover:bg-white/30 backdrop-blur-sm transition-colors text-white font-semibold px-6 py-2 rounded-full shadow-md focus:outline-none" onClick={() => setOpen((prev) => !prev)}>
        Mi cuenta
      ...
      <div className="flex items-center justify-between relative h-full">
        ...
        <div className="z-20">
          <span className="text-white font-extrabold text-3xl tracking-wider transition cursor-pointer" onClick={() => navigate("/Home-i")}>
            <img src="/logo1.png" alt="logo" className="w-48 md:w-64 h-auto self-center shadow-lg rounded-full"/>
          </span>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-8 z-30 absolute left-1/2 transform -translate-x-1/2">
          ...
          <button onClick={() => navigate("/Matcha/New-idea")} className="text-white font-semibold px-6 py-2 rounded-full hover:bg-white/20 transition-colors">
            Generar Idea
          </button>
          <button onClick={() => navigate("/Matcha/Nuevo-proyecto")} className="text-white font-semibold px-6 py-2 rounded-full hover:bg-white/20 transition-colors">
            Generar Proyecto
          </button>
          <button onClick={() => navigate("/Matcha/My-projects")} className="text-white font-semibold px-6 py-2 rounded-full hover:bg-white/20 transition-colors">
            Match histórico
          </button>
          <button onClick={() => navigate("/Matcha/Select-Project")} className="text-white font-semibold px-6 py-2 rounded-full hover:bg-white/20 transition-colors">
            Match fondos
          </button>
        </nav>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-[#9ab05f] shadow-lg z-40">
          <div className="px-6 py-4 space-y-1">
            ...
            <button onClick={() => {navigate("/Matcha/New-idea"); setIsMobileMenuOpen(false);}} className="block w-full text-left text-white font-semibold px-4 py-3 rounded-lg hover:bg-white/20 transition-colors">
              Generar Idea
            </button>
            <button onClick={() => {navigate("/Matcha/Nuevo-proyecto"); setIsMobileMenuOpen(false);}} className="block w-full text-left text-white font-semibold px-4 py-3 rounded-lg hover:bg-white/20 transition-colors">
              Generar Proyecto
            </button>
            <button onClick={() => {navigate("/Matcha/My-projects"); setIsMobileMenuOpen(false);}} className="block w-full text-left text-white font-semibold px-4 py-3 rounded-lg hover:bg-white/20 transition-colors">
              Match histórico
            </button>
