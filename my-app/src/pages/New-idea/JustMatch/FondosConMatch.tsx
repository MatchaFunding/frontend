...
const SearchIcon: React.FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke={colorPalette.oliveGray} className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" /></svg>
);
    ...
    <div onContextMenu={handleContextMenu} className="bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col border border-slate-200/80 cursor-pointer h-[560px]">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="mt-auto border-t border-slate-200 pt-4 space-y-3">
          <div className="flex items-center text-sm text-slate-600"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2 text-slate-400"><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0h18M-4.5 12h22.5" /></svg><strong>Cierre:</strong><span className="ml-2">{formatDate(FechaDeCierre)}</span></div>
          {MontoMaximo > 0 && (<div className="flex items-center text-sm text-slate-600"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2 text-slate-400"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg><strong>Monto Máx:</strong><span className="ml-2">{new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP' }).format(MontoMaximo)}</span></div>)}
          <div className="flex justify-start"><span className="bg-[rgba(139,168,136,0.2)] text-[rgba(68,98,74,1)] text-xs font-semibold px-3 py-1 rounded-full">{TipoDeBeneficio}</span></div>
        </div>
        <button onClick={() => navigate(`/Matcha/Select-Project/fondos/detalle/${ID}`)} className="w-full bg-[#8ba888] hover:bg-[rgba(68,98,74,0.8)] text-white font-bold py-3 px-4 rounded-xl transition-colors duration-300 mt-4">Ver más detalles</button>
      </div>
    </div>
  );
};
...
const FondosconPorcentaje: React.FC = () => {
  ...
  const [tipoBeneficioFilter, setTipoBeneficioFilter] = useState('Todas'); // CORREGIDO
  ...
  // Estado y constante para la paginación
  ...
  useEffect(() => {
    const CargarFondosYCalces = async () => {
      setIsLoading(true);
      setError(null);
      let project: Proyecto | undefined;
      ...
      if (projectData) {
        try {
          ...
          setError("No se pudo leer el proyecto seleccionado.");
          setIsLoading(false);
          return;
        }
      }
      try {
        ...
        if (project && project.ID) {
          ...
          const fondosConCompatibilidad = listaDeFondos.map(fondo => {
            ...
            return { ...fondo, compatibilidad: match ? Math.floor((match.affinity || 0) * 100) : 0, semantic_score: match?.semantic_score, topic_score: match?.topic_score, rules_score: match?.rules_score };
          });
  ...
  const tiposDeBeneficio = useMemo(() => {
    return ['Todas', ...new Set(fondos.map(f => f.TipoDeBeneficio).filter(Boolean))];
  }, [fondos]);
  ...
  const filteredFondos = useMemo(() => {
    ...
    if (tipoBeneficioFilter !== 'Todas') { // CORREGIDO
      fondosFiltrados = fondosFiltrados.filter(f => f.TipoDeBeneficio === tipoBeneficioFilter); // CORREGIDO
    }
  ...
  if (isLoading) {
    return (<div className="min-h-screen bg-[#f1f5f9] flex flex-col"><NavBar /><div className="flex flex-col items-center justify-center flex-1 space-y-6"><LoopAnimation /><p className="text-xl sm:text-2xl font-semibold text-gray-700 animate-pulse">Buscando los mejores fondos para ti...</p></div></div>);
  }
  if (error) {
    return (<div className="min-h-screen bg-[#f1f5f9] flex flex-col"><NavBar /><div className="flex-grow flex items-center justify-center text-center px-4"><div className="bg-red-50 border-l-4 border-red-500 p-4 rounded"><p className="text-lg text-red-700">Ups, algo salió mal</p><p className="text-sm text-red-600 mt-1">{error}</p></div></div></div>);
  }
    ...
    <div className="min-h-screen bg-[#f1f5f9]">
      ...
      <main className="flex-grow p-6 md:p-10 max-w-screen-2xl mx-auto mt-[5%]">
        <div className="text-center mb-10">
          {selectedProject ? (<><h1 className="text-4xl font-bold text-[#505143]">Fondos Recomendados para</h1><h2 className="text-3xl font-semibold text-[#44624a] mt-1">"{selectedProject.nombre}"</h2></>) : (<h1 className="text-4xl font-bold text-[#505143]">Explorar Todos los Fondos</h1>)}
        </div>
        <header className="bg-white p-4 sm:p-6 rounded-2xl shadow-md mb-8 flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <select
              value={tipoBeneficioFilter} 
              onChange={(e) => setTipoBeneficioFilter(e.target.value)} 
            ...
            <div className="relative">
              ...
              {isDropdownOpen && (<div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl z-10">
        ...
        {!selectedProject && (<div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-md mb-8"><p className="font-bold">No has seleccionado un proyecto.</p><p>Mostrando todos los fondos disponibles. Para ver recomendaciones personalizadas, <Link to="/Matcha/Select-Project" className="underline font-semibold">selecciona un proyecto primero</Link>.</p></div>)}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {paginatedFondos.length > 0 ? (paginatedFondos.map(fondo => (<FondoCard key={fondo.ID} {...fondo} onRightClick={handleCardRightClick} />))) : (<div className="col-span-full text-center py-12"><p className="text-xl text-gray-500">No se encontraron fondos que coincidan con los filtros.</p></div>)}
        </div>
        <footer className="flex flex-col sm:flex-row justify-between items-center mt-10 text-[rgba(80,81,67,1)]">
          ...
          {totalPages > 1 && (<div className="flex items-center gap-2 mt-4 sm:mt-0">
              <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="px-3 py-1 border rounded-lg hover:bg-[rgba(139,168,136,0.2)] disabled:opacity-50 disabled:cursor-not-allowed">&lt;</button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(pageNumber => (<button key={pageNumber} onClick={() => setCurrentPage(pageNumber)} className={`px-3 py-1 border rounded-lg ${currentPage === pageNumber ? 'bg-[rgba(68,98,74,1)] text-white border-[rgba(68,98,74,1)]' : 'hover:bg-[rgba(139,168,136,0.2)]'}`}>{pageNumber}</button>))}
              <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="px-3 py-1 border rounded-lg hover:bg-[rgba(139,168,136,0.2)] disabled:opacity-50 disabled:cursor-not-allowed">&gt;</button>
          </div>)}
        </footer>
      </main>
